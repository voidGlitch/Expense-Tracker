...
export default function SharedExpensesView() {
  ...
  return <div className="mx-auto max-w-3xl space-y-5 pb-16">
    <section className="rounded-[2rem] bg-gradient-to-br from-teal-800 via-teal-700 to-cyan-700 p-5 text-white shadow-lg shadow-teal-700/25 sm:p-7"><p className="text-sm font-medium text-teal-50">Shared expenses</p><div className="mt-1 flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-2xl font-extrabold tracking-tight text-white sm:text-3xl">{user?.name?.split(' ')[0] || 'Your'} balances</h1><p className="mt-1 text-sm font-medium text-teal-50">Add an expense, track debts, and settle fairly.</p></div><div className="flex w-full flex-wrap gap-2 sm:w-auto"><Button className="flex-1 !bg-white !text-teal-800 shadow-sm hover:!bg-teal-50 sm:flex-none" icon={Plus} disabled={!canAddExpense} title={canAddExpense ? undefined : 'Create a group or add a friend first'} onClick={() => setAdding(true)}>Add expense</Button><Button className="flex-1 !border !border-white/60 !bg-teal-950/30 !text-white hover:!bg-white/15 sm:flex-none" icon={CirclePlus} onClick={() => setNewGroup(true)}>Add group</Button><Button className="flex-1 !border !border-white/60 !bg-teal-950/30 !text-white hover:!bg-white/15 sm:flex-none" icon={Plus} onClick={() => setAddFriend(true)}>Add friend</Button></div></div></section>
