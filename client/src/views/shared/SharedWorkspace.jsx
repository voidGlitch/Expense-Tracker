...
export default function SharedWorkspace() {
  ...
  const title = context?.title || friend?.user?.name || ({ dashboard: 'Shared Expenses', activity: 'Recent activity', payments: 'Payments', friends: 'Friends', groups: 'Groups' }[selection.type]) || 'Shared Expenses';
  ...
  const events = useMemo(() => {
    ...
    return selectedContexts.flatMap((c) => [
      ...
      if (selection.type === 'payments' && row.entryType !== 'payment') return false;
  ...
  }, [selectedContexts, friend, me.id, query, currency, showDeleted, selection.type]);
  ...
  const settleCandidates = data.friends.filter((candidate) => Object.values(candidate.byCurrency || {}).some((amount) => minor(amount) !== 0));
  const openCandidatePayment = (candidate) => setDialog({ type: 'payment', friend: candidate });
  ...
  return <div className="space-y-3 pb-10">
    ...
    <div className="shared-workspace">
      ...
      <main className="shared-main"><header className="shared-header"><div><h1>{title}</h1>{context && <p className="text-xs shared-neutral">{context.memberIds.length} members{context.entity.archivedAt ? ' · Archived' : ''}</p>}{friend?.user?.isGuest && <p className="text-xs shared-neutral">Guest · links when this email registers</p>}</div><div className="shared-toolbar">
        <Button icon={Plus} disabled={!activeContexts.length || Boolean(context?.entity.archivedAt || context?.readOnly)} onClick={openExpense}>Add expense</Button><Button variant="secondary" onClick={() => setDialog({ type: 'new-group' })}>Add group</Button><Button variant="secondary" onClick={() => setDialog({ type: 'friend' })}>Add friend</Button>{selection.type === 'payments' && settleCandidates[0] && <Button variant="secondary" onClick={() => openCandidatePayment(settleCandidates[0])}>Settle up</Button>}
      ...
      {selection.type === 'payments' && <PaymentOverview friends={settleCandidates} openPayment={openCandidatePayment} />}
      {(context || friend || selection.type === 'activity' || selection.type === 'payments') && <>
        ...
        <div className="flex flex-wrap items-center gap-3 border-b p-3 text-xs"><span className="shared-neutral">{selection.type === 'payments' ? 'All recorded payments' : 'All shared activity'}</span></div>
      ...
      <nav className="shared-mobile-nav" aria-label="Shared mobile navigation"><button onClick={() => select('friends')}>Friends</button><button onClick={() => select('groups')}>Groups</button><button className="bg-transparent p-1 text-white" aria-label="Add shared expense" title="Add shared expense" disabled={!activeContexts.length} onClick={openExpense}><img src="/icons/flaticon-plus-4315609.png" alt="" width="30" height="30" /></button><button onClick={() => select('activity')}>Activity</button><button onClick={() => select('payments')}>Payments</button></nav>
    </div>
...
function PaymentOverview({ friends, openPayment }) { return <section aria-label="Payments overview" className="shared-payment-overview"><div className="border-b p-4"><h2 className="font-bold">Who owes whom</h2><p className="mt-1 text-xs shared-neutral">Positive amounts are owed to you. Negative amounts are what you owe.</p></div>{friends.length ? friends.map((friend) => <div className="shared-row" key={friend.id}><Avatar name={friend.user?.name} /><div className="min-w-0 flex-1"><p className="font-semibold">{friend.user?.name}</p><small>{friend.user?.isGuest ? 'Guest participant' : 'Registered account'}</small></div><div className="text-right text-sm">{Object.entries(friend.byCurrency).filter(([, amount]) => minor(amount) !== 0).map(([currency, amount]) => <p key={currency} className={minor(amount) > 0 ? 'shared-positive' : 'shared-negative'}><b>{minor(amount) > 0 ? '+' : '−'}{formatMoney(Math.abs(amount), currency)}</b><small className="block">{minor(amount) > 0 ? 'owes you' : 'you owe'}</small></p>)}</div><Button variant="secondary" size="sm" onClick={() => openPayment(friend)}>Settle up</Button></div>) : <p className="p-8 text-center shared-neutral">Everyone is settled up.</p>}</section>; }
