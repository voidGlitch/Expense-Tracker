...
export default function SharedWorkspace() {
  ...
  return <div className="space-y-3 pb-10">
    ...
    <div className="shared-workspace">
      ...
      <nav className="shared-mobile-nav" aria-label="Shared mobile navigation"><button onClick={() => select('friends')}>Friends</button><button onClick={() => select('groups')}>Groups</button><button className="rounded-full bg-teal-600 px-4 text-white" aria-label="Add shared expense" disabled={!activeContexts.length} onClick={openExpense}><Plus size={20} /></button><button onClick={() => select('activity')}>Activity</button><a href="/settings">Account</a></nav>
    </div>
